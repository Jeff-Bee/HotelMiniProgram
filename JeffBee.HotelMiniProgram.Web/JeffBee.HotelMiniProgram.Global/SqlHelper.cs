...
namespace JeffBee.HotelMiniProgram.Global
{
    public static class SqlHelper
    {
        public static bool Insert<T>(T parameter, string connectionString) where T : class
        {
            ...
            {
                Logger.LogError(string.Format("Insert:{0}异常!", typeof(T)));
                Debug.WriteLine(string.Format("Insert:{0}异常!", typeof(T)));
        ...
        public static IList<T> GetList<T>(string sql,string connectionString) where T : class
        {
            using (var sqlConnection = new OleDbConnection(connectionString))
            {
                sqlConnection.Open();
                var result = sqlConnection.GetList<T>(sql);
                sqlConnection.Close();
                return result.ToList();
            }
        }
        public static IList<T> GetList<T>(PredicateGroup predicate,string connectionString) where T : class
        {
            ...
            {
                sqlConnection.Open();
                var result = sqlConnection.GetList<T>(predicate);
                sqlConnection.Close();
                return result.ToList();
            }
        }
        ...
        /// <param name="connectionString"></param>
        /// <returns></returns>
        public static bool Execute(string storedProcedure, dynamic param = null,
            ...
            int? commandTimeout = null, string connectionString = null)
        {
            try
            {
                ...
                {
                    sqlConnection.Open();
                    sqlConnection.Execute(storedProcedure, param: (object)param, transaction: transaction,  commandTimeout: commandTimeout, commandType: CommandType.StoredProcedure);
                    return true;
                }
            }
            catch (Exception ex)
            {
                Logger.LogError(String.Format("执行存储过程出错:\r\n{0}", storedProcedure), "AppLogger");
        ...
        public static bool Execute(string sql, string connectionString)
        {
            ...
            {
                Logger.LogError(String.Format("执行SQL语句出错:\r\n{0}",sql), "AppLogger");
                Logger.LogError4Exception(ex, "AppLogger");
                return false;
            }
        ...
        /// <summary>
        /// 执行SQL语句，返回float，如果失败返回null
        /// </summary>
        /// <param name="sql"></param>
        /// <param name="connectionString"></param>
        /// <returns></returns>
        public static float? ExecuteScalar2Float(string sql, string connectionString)
        {
            float value = float.MinValue;
            object ret = ExecuteScalar(sql, connectionString);

            if (ret != null && float.TryParse(ret.ToString(), out value))
            {
                return value;
            }
            else
        ...
        public static OleDbDataReader GetReader(string connectionString, string commandText)
        {
            // 设定参数数组为null，调用重载方法
            return GetReader(connectionString, CommandType.Text, commandText, (OleDbParameter[])null);
        }
        /// <summary>
        ...
        /// <param name="commandText">文本（存储过程名或者T-SQL命令）</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(string connectionString, CommandType commandType, string commandText)
        {
            // 设定参数数组为null，调用重载方法
            return GetReader(connectionString, commandType, commandText, (OleDbParameter[])null);
        }
        ...
        /// <param name="commandParameters">参数数组</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(string connectionString, CommandType commandType, string commandText, params OleDbParameter[] commandParameters)
        {
        ...
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(string connectionString, string spName, params object[] parameterValues)
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
        ...
        /// <param name="commandText">文本（存储过程名或者T-SQL命令）</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(OleDbConnection connection, CommandType commandType, string commandText)
        {
            // 设定参数数组为null，调用重载方法
            return GetReader(connection, commandType, commandText, (OleDbParameter[])null);
        }
        ...
        /// <param name="commandParameters">参数数组</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(OleDbConnection connection, CommandType commandType, string commandText, params OleDbParameter[] commandParameters)
        {
            // 调用重载方法，事务参数为null，指定数据库对象由调用者管理
            return GetReader(connection, (OleDbTransaction)null, commandType, commandText, commandParameters, SqlConnectionOwnership.External);
        }
        ...
        /// <param name="parameterValues">值数组</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(OleDbConnection connection, string spName, params object[] parameterValues)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
        ...
        /// <param name="commandText">文本（存储过程名或者T-SQL命令）</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(OleDbTransaction transaction, CommandType commandType, string commandText)
        {
            // 设定参数数组为null，调用重载方法
            return GetReader(transaction, commandType, commandText, (OleDbParameter[])null);
        }
        ...
        /// <param name="commandParameters">参数数组</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(OleDbTransaction transaction, CommandType commandType, string commandText, params OleDbParameter[] commandParameters)
        {
        ...
        /// <param name="parameterValues">值数组</param>
        /// <returns>返回保存执行结果的DataReader</returns>
        public static OleDbDataReader GetReader(OleDbTransaction transaction, string spName, params object[] parameterValues)
        {
            if (transaction == null) throw new ArgumentNullException("transaction");
            if (transaction != null && transaction.Connection == null) throw new ArgumentException("The transaction was rollbacked or commited, please provide an open transaction.", "transaction");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
        ...
        private static void PrepareCommand(OleDbCommand command, OleDbConnection connection, OleDbTransaction transaction, CommandType commandType, string commandText
            , OleDbParameter[] commandParameters, out bool mustCloseConnection, int commandTimeout /*= 30*/)
        {
            if (command == null) throw new ArgumentNullException("command");
